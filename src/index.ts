 ...
 * lives in the closure created by this function. Two daemons hosting
 * this plugin no longer share module-level state.
 */
export const createPlugin: VibePluginFactory = (
  _ctx: ProfileContext,
): VibePlugin => {
  // ── Closure-bound state (was module-level) ──────────────────────────
  ...
  const getPromptDb = (): PromptDatabase =>
    requireDb(promptDb, "PromptDatabase");
  const getContextDb = (): ContextDatabase =>
    requireDb(contextDb, "ContextDatabase");
  const getSessionDb = (): SessionDatabase =>
    requireDb(sessionDb, "SessionDatabase");
  const getLogDb = (): LogDatabase => requireDb(logDb, "LogDatabase");
  const getDispatchDb = (): DispatchedPromptDatabase =>
    requireDb(dispatchDb, "DispatchedPromptDatabase");
  const getQueueDb = (): QueueDatabase => requireDb(queueDb, "QueueDatabase");
  const getFileDb = (): FileDatabase => requireDb(fileDb, "FileDatabase");

  const getAIProvider = (agentType: string): unknown | undefined =>
    hostServicesRef?.serviceRegistry?.getProviderByName("ai", agentType);

  const listAIProviders = (): AIProviderSummary[] => {
  ...
  const pickFirstSessionProvider = (): SessionProviderLike | undefined => {
    const registry = hostServicesRef?.serviceRegistry;
    ...
    onCliSetup(program: Command, hostServices?: HostServices) {
      ...
      aiCmd
        ...
        .action((toolName: string, options: { json?: boolean }) => {
          ...
          const { installed: alreadyInstalled, version } =
        ...
        .action(
          async (
            ...
            if (!tool) {
              ...
              console.error(
                `\x1b[31mError:\x1b[0m Unknown tool '${toolName}'.`,
              );
              process.exit(1);
            }
            ...
            if (segments.length > 1) {
              mkdirSync(join(dir, ...segments.slice(0, -1)), {
                recursive: true,
              });
            }
          ...
          async (id: string, options: { json?: boolean; plain?: boolean }) => {
            await runMultimode<ReturnType<PromptDatabase["getById"]>>({
              ...
              fetchData: () => {
                ...
                if (!prompt) {
                  console.error(
                    `\x1b[31mError:\x1b[0m Prompt not found: ${id}`,
                  );
                  process.exit(1);
                }
                return prompt;
          ...
          async (id: string, options: { json?: boolean; plain?: boolean }) => {
            await runMultimode<ReturnType<ContextDatabase["getById"]>>({
              ...
              fetchData: () => {
                ...
                if (!ctx) {
                  console.error(
                    `\x1b[31mError:\x1b[0m Context not found: ${id}`,
                  );
                  process.exit(1);
                }
                return ctx;
