 ...
 *
 * Upload, list, and delete file attachments for AI sessions.
 * Files are stored at ~/.vibecontrols/ai-files/<sessionId>/.
...
const FILES_BASE_DIR = join(os.homedir(), ".vibecontrols", "ai-files");

function ensureSessionDir(sessionId: string): string {
  const dir = join(FILES_BASE_DIR, sessionId);
  if (!existsSync(dir)) {
