...
// We dynamically import @opentui/core at runtime — typed loosely so this
// plugin doesn't need a hard dependency on @opentui/core types.
// eslint-disable-next-line @typescript-eslint/no-explicit-any
type AnyCtor = new (ctx: unknown, opts: Record<string, unknown>) => any;
// eslint-disable-next-line @typescript-eslint/no-explicit-any
type AnyRenderer = any;

interface OpenTuiCore {
  createCliRenderer: (cfg: Record<string, unknown>) => Promise<AnyRenderer>;
  BoxRenderable: AnyCtor;
  TextRenderable: AnyCtor;
  SelectRenderable: AnyCtor;
...
async function loadCore(): Promise<OpenTuiCore> {
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  return (await import("@opentui/core" as any)) as unknown as OpenTuiCore;
}
